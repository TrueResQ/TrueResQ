...
function Form({ heading, headingCenter, formDetails, children }: FormProps) {
  return (
    <div className="w-11/12 px-4 sm:px-6 lg:px-8 z-0 bg-background-secondary text-white">
      <p className={`text-lg font-bold ${headingCenter ? "text-center" : ""}`}>{heading}</p>
      <div className="p-8 mt-6 mb-0 space-y-4 rounded-lg bg-background">
        <form action="" className="pb-6">
          {formDetails.map((formDetail) => (
            <div key={formDetail.label}>
              <label className="text-sm font-medium text-white">{formDetail.label}</label>
              <div className="relative mt-1">
                {formDetail.onChange ? (
                  <input
                    type="email"
                    id="email"
                    className="w-full p-4 pr-12 text-sm bg-background-secondary text-white rounded-lg shadow-sm z-0"
